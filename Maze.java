...
class Maze {
	...
	public ArrayList<Square> storePath() {
		...
		// Empty Array for returning if there is no path
		ArrayList<Square> emptyArray =  new ArrayList<Square>();
		// An ArrayList that returns path
		ArrayList<Square> returner = new ArrayList<Square>();
		// initializing current as finish square
		Square current = this.finish;
		
        /*
         * This while loop continues until we reach the starting point
         * if there is no path from start to finish (null), it returns 
         * Empty Array otherwise it adds each element from finish to one before
         * the starting point in returner
         * It adds all the element in the returner as the first element
         *  
         */
		while(current != this.start) {
			...
			if(current == null) {
				...
				returner.add(0, current);
				current = current.getPrevious();
			}
		}
		
		//adding starting point to the returner as first element
		returner.add(0, current);
